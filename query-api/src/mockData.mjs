const runtimeTypes = ['runc', 'gvisor', 'kata', 'firecracker'];
...
export function filterSandboxes(query) {
...
export function metricsForSandbox(sandboxId) {
...
export function eventsForSandbox(sandboxId) {
  const target = sandboxes.find((item) => item.id === sandboxId) ?? sandboxes[0];
  const base = new Date(target.createdAt).getTime();
  const event = (offsetMs, severity, eventName, message, attributes = {}) => ({
  ...
  if (target.runtimeType === 'gvisor') rows.push(event(target.startupDurationMs + 80_000, 'warning', 'gvisor.sentry.cpu.high', 'gVisor sentry CPU stayed above baseline', { sentryCpu: '38%' }));
  if (target.nodeId === 'node-b') rows.push(event(target.startupDurationMs + 25_000, 'warning', 'node.io.pressure.high', 'Node IO pressure is elevated', { psiSome: '0.78' }));
...
export function traceForSandbox(sandboxId) {
  const target = sandboxes.find((item) => item.id === sandboxId) ?? sandboxes[0];
  const start = new Date(target.createdAt).getTime();
  const total = target.startupDurationMs;
  const img = imageById(target.imageId);
  const pull = img.sizeBytes > 2 * 1024 ** 3 ? total * 0.18 : total * 0.14;
  const unpack = img.layerCount > 60 ? total * 0.52 : total * 0.22;
...
export function profilesForSandbox(sandboxId) {
  const target = sandboxes.find((item) => item.id === sandboxId) ?? sandboxes[0];
...
function sandbox(id, runtimeType, nodeId, imageId, workloadName, status, startupDurationMs, cpuAvg, memoryPeakBytes, eventCount, attributes = {}) {
...
function image(id, ref, digest, loadingMode, sizeBytes, layerCount, warmBlockRatio, startupCostMs) {
  return {
    id,
    ref,
    digest,
...
function imageDownloadTimeline(imageId, imageSizeBytes, layerCount, startupCostMs) {
...
function imageLayers(imageId, imageSizeBytes, layerCount, warmBlockRatio, startupCostMs) {
  ...
  return commands.map((command, index) => {
    ...
    const remotePenalty = 0.25 + ((requestedBlockCount - cacheHitBlockCount) / requestedBlockCount);
...
function series(target, name, label, unit, group, base, wave, spike) {
...
function cpuFlamegraph(runtimeType, role) {
  if (runtimeType === 'gvisor') {
    return frame(`${role}:root`, 100, [
      frame('sentry/syscalls.handle', 42, [frame('fs/gofer.walk', 17), frame('netstack/tcp.dispatch', 14), frame('security/seccomp.check', 11)]),
...
function ioFlamegraph(highPressure) {
...
function frame(name, value, children) {
  return { name, value, children };
}

function imageById(id) {
  const img = images.find((item) => item.id === id);
  if (!img) throw new Error(`Missing mock image: ${id}`);
  return img;
}

function minutesAgo(minutes, now = new Date('2026-05-09T04:00:00.000Z')) {
  return new Date(now.getTime() - minutes * 60_000).toISOString();
}

export function isRuntimeType(value) {
  return runtimeTypes.includes(value);
}
