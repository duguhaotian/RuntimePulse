...
- CRI+containerd startup collector. The lightweight event-correlated path is implemented through `cri-startup-trace`, using CRI sandbox ready events and containerd create/task-start events to produce `sandbox.startup.e2e` traces. The `startup-callchain` report bridge now accepts detailed RunPodSandbox/CNI/OCI/Kata spans and raw uprobe/eBPF enter/exit events from real exporters, then derives per-CNI-plugin and OCI/Kata/helper metrics. A minimal real `runtimepulse-startup-probe` bridge is available under `tools/`, using bpftrace exec/exit tracepoints to emit raw call-chain events for CNI plugin binaries, OCI/Kata runtime binaries, containerd shims, and optional helper binaries. Next depth is in-tree/native eBPF uprobe profiles for containerd CRI `RunPodSandbox` and OCI-runtime boundaries; plain process exec tracing alone is not considered sufficient for concurrent RunPodSandbox correlation, and CRI proxying is not the target design.
