...
1. Complete non-Kubernetes P1 collector depth first, with Kata, Firecracker, and gVisor sandbox report collectors ahead of remaining Kubernetes/containerd production depth.
2. Continue snapshotter/image-cache fidelity, runtime diagnostics, and native profiling adapter depth after the sandbox runtime report collectors. eBPF-specific deepening is explicitly deferred until these sandbox runtime paths are usable.
...
- Query API also treats CRI startup traces (`sandbox.startup.e2e`) and startup call-chain reports/metrics as authoritative startup-duration sources, so detailed RunPod/CNI/OCI/Kata measurements update sandbox startup summaries instead of remaining raw metrics only.
...
- Rust host-agent can run `cri-startup-trace`/`host-cri-startup-trace` to correlate CRI `SANDBOX_CREATED`/`SANDBOX_READY` events with containerd container/task lifecycle events and emit event-based CRI+containerd `sandbox.startup.e2e` traces plus startup duration metrics for runc/Kata validation without deploying a CRI proxy.
- Rust host-agent can run `startup-callchain`/`host-startup-callchain` to ingest real RunPodSandbox/CNI/OCI/Kata call-chain JSON or JSONL reports from an external uprobe/eBPF exporter command or file, including raw enter/exit uprobe events paired by request/correlation id with pairing-quality metrics, and normalize them into trace spans, derived CNI/OCI/helper-binary startup metrics, per-CNI-plugin startup metrics, and observation events.
- Kubernetes/containerd remains available but is behind the current non-Kubernetes P1 collector work. Docker remains the single-node/local validation path, while CRI/Kubelet event ingestion enriches Kubernetes lifecycle context around containerd inventory and events.
...
- Docker and containerd periodic inventory no longer emit per-container `*.container.observed` events; inventory updates remain metadata, while lifecycle/event streams own timeline events.
- Node and sandbox metric samplers no longer emit periodic `*.sample.observed` events; metrics carry the samples, and sandbox active-set reconciliation is driven by metadata snapshot attributes.
- Containerd inventory publishes `containerd-running` metadata snapshots, including empty snapshots, so Query API can remove stale containerd/Kubernetes sandboxes even when lifecycle events are missed.
- Containerd sandbox cgroup sampling refreshes its active target set from containerd task inventory on each collection cycle, while containerd events still provide fast start/stop updates.
...
- CRI+containerd startup collector. The lightweight event-correlated path is implemented through `cri-startup-trace`, using CRI sandbox ready events and containerd create/task-start events to produce `sandbox.startup.e2e` traces. The `startup-callchain` report bridge now accepts detailed RunPodSandbox/CNI/OCI/Kata spans and raw uprobe/eBPF enter/exit events from real exporters, then derives per-CNI-plugin, per-process-binary, OCI/Kata/helper metrics. Query API analysis and the Sandbox detail UI use those process-binary breakdowns to identify the hottest startup binary and helper binary. A minimal real `runtimepulse-startup-probe` bridge is available under `tools/`, using bpftrace exec/exit tracepoints to emit raw call-chain events for CNI plugin binaries, OCI/Kata runtime binaries, containerd shims, and optional helper binaries. Optional containerd CRI `RunPodSandbox` Go uprobes now decode request pod identity on amd64 and match it back to the real sandbox id through CNI env identity in concurrent runc/Kata captures. CNI/CNI_ARGS and RunPod request identity are emitted as both attributes and top-level report fields so normalization can derive stable `k8s-{namespace}-{pod}-pod` ids; OCI/Kata exec/uprobe boundaries carry exact shim-argv, bundle, or PID correlation markers. The validation helper now runs real workload `crictl create/start` after RunPodSandbox for both runc and Kata, verifies `startup.phase=container` events stay attached to the pod sandbox, and preserves the raw workload task id through `startup.workload_container_id`/`containerd.raw_id`. CNI plugin cost is authoritative from plugin `execve` plus `CNI_CONTAINERID`/`CNI_ARGS`; containerd CNI setup uprobes are debug-only boundaries and stay pending when they cannot be attributed exactly. Plain process exec tracing alone is not considered sufficient for concurrent RunPodSandbox correlation, and CRI proxying is not the target design.
- Kubernetes/containerd event collector. Containerd inventory and container/task/content/snapshot event streaming are available for the later Kubernetes path, especially the `k8s.io` namespace. CRI/Kubelet events are used as lifecycle enrichment.
...
- image metadata and cache collector. Docker image metadata/layer breakdown and containerd content-store image enrichment are implemented; Docker image pull/tag/delete and containerd content/snapshot timeline observations come from runtime event dispatch; precise pull sub-stage durations and lazy block-cache hit curves are ingested from real snapshotter/exporter JSON reports through `image-cache`. Snapshotter reports now also accept common snapshotter state/index JSON and JSONL, Prometheus text exposition, layer-level cache counters, and prefetch records, deriving aggregate lazy-layer and prefetch metrics plus image timeline spans. When only per-layer cache counters are present, image-level lazy cache metrics are synthesized from layer totals and marked with `snapshotter.metricSource=layers`; host-agent can invoke a real image-cache exporter command on each tick.
...
- eBPF/profile collector. Generic profile artifact report ingestion is implemented with target/stats metadata, artifact-index alias parsing, profile observation events, profile metric derivation, and perf/eBPF exporter command hooks; eBPF-specific deepening is deferred behind Kata/Firecracker and remaining non-Kubernetes P1 runtime work.
- Diagnostic artifact collector. Generic diagnostic report ingestion is implemented for support-bundle/log/inspect indexes, including artifact-index alias parsing plus host-agent file and command exporter hooks; Docker, crictl, and native containerd diagnostics exporters are now available; artifact UI workflows plus native perf-script conversion with target/duration inference and perf/eBPF folded-stack conversion with command input is available; next work is broader backend-specific profiler depth.
...
- Sandbox detail overview now shows a RunPod startup call-chain summary for CNI plugin/OCI/Kata/helper phases, including a per-CNI-plugin binary breakdown and the dominant plugin label, plus rule findings with evidence, related metrics, recommended actions, and trace-span drill-down links.
