...
- Rust host-agent can run `startup-callchain`/`host-startup-callchain` to ingest real RunPodSandbox/CNI/OCI/Kata call-chain JSON or JSONL reports from an external uprobe/eBPF exporter command or file and normalize them into trace spans, derived CNI/OCI/helper-binary startup metrics, per-CNI-plugin startup metrics, and observation events.
...
- CRI+containerd startup collector. The lightweight event-correlated path is implemented through `cri-startup-trace`, using CRI sandbox ready events and containerd create/task-start events to produce `sandbox.startup.e2e` traces. The `startup-callchain` report bridge now accepts detailed RunPodSandbox/CNI/OCI/Kata spans from real exporters. Next depth is native eBPF uprobe profiles for containerd CRI `RunPodSandbox` and OCI-runtime boundaries plus CNI/helper-binary attribution; plain process exec tracing alone is not considered sufficient for concurrent RunPodSandbox correlation, and CRI proxying is not the target design.
