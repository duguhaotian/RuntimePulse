...
- Rust host-agent can run `startup-callchain`/`host-startup-callchain` to ingest real RunPodSandbox/CNI/OCI/Kata call-chain JSON or JSONL reports from an external uprobe/eBPF exporter command or file and normalize them into trace spans, derived CNI/OCI/helper-binary startup metrics, and observation events.
