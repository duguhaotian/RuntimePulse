...
1. Deepen non-Kubernetes P1 collector data first: snapshotter/image-cache fidelity, native profiling adapter hooks, and diagnostic artifact workflows. Generic diagnostic-report ingestion is now in place for support-bundle/log/inspect artifact indexes from files or host-agent exporter commands.
